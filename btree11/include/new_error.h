...
ERROR MESSAGES:
  ...
  a static "ErrorStringTable" object.  The constructor of this object
  registers the error messages with the system when the program first starts.
  Here is the buffer manager's ErrorStringTable declaration, in the same file
  as the above array of strings:

     static ErrorStringTable bufTable( BUFMGR, bufErrMsgs );
...
HANDLING ERRORS:
  ...
  record in the list; you may then use methods of the ErrorNode class to get
...
/* This is how you register the error messages for a subsystem.  You declare
   a static ErrorStringTable object and pass the constructor the subsystem
   ...
   static ErrorStringTable errtable( BUFMGR, bufErrMsgs );

   Here "bufErrMsgs" is previously declared as a static array of strings.
   */

class ErrorStringTable
{
	private:
		static const char** table[];

	public:
		ErrorStringTable( Status subsystem, const char* messages[] ) { table[subsystem] = messages; }
...
class ErrorNode
{
	...
	public:
		ErrorNode( Status subsys, Status prior = OK, int err_index = -1,
				const char* extra_msg = NULL );

		~ErrorNode();
		void set_next(ErrorNode* nxt)       { next_node = nxt; }

		void show_error( ostream& to=cerr ) const;
		static const char* team_name(Status T1);

		const ErrorNode* get_next() const   { return next_node; }
		...
		{ return ErrorStringTable::get_message(subsystem, error_index); }
...
{
	private:
		Status add_error( ErrorNode* next );
	...
	public:
		...
		{ return add_error( new ErrorNode(subsystem,OK,-1,msg) ); }
...
extern GlobalErrors minibase_errors;
